...
virtio_disk_init(void)
{
  ...
  u32 magic = *R(VIRTIO_MMIO_MAGIC_VALUE);
  u32 version = *R(VIRTIO_MMIO_VERSION);
  u32 device_id = *R(VIRTIO_MMIO_DEVICE_ID);
  u32 vendor_id = *R(VIRTIO_MMIO_VENDOR_ID);
  ...
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
  ...
  char *queue_mem = (char *)alloc_page();
  if(queue_mem == 0)
    panic("virtio disk alloc_page");

  mem_set((addr_t)queue_mem, 0, PAGE_SIZE);
  ...
  char *used_addr = (char *)(((u64)avail_end + VRING_ALIGN - 1) & ~(u64)(VRING_ALIGN - 1));
  disk.used = (struct virtq_used *)used_addr;

  if(used_addr + sizeof(struct virtq_used) > queue_mem + PAGE_SIZE)
    panic("virtio ring too large");

  *R(VIRTIO_MMIO_GUEST_PAGE_SIZE) = PAGE_SIZE;
  *R(VIRTIO_MMIO_QUEUE_ALIGN) = VRING_ALIGN;
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
  *R(VIRTIO_MMIO_QUEUE_PFN) = ((u64)queue_mem) >> 12;
...
{
  u32 irq_status = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
  if(irq_status)
