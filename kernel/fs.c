...
struct inode* ialloc(u64 dev, short type) {
    ...
    for(inum = 1; inum < sb.ninodes; inum++) {
        ...
        if(dip->type == 0) {  // 空闲 inode
            mem_set((addr_t)dip, 0, sizeof(*dip));
            dip->type = type;
            log_write(bp);
            brelse(bp);
...
void ilock(struct inode *ip) {
    ...
    if(ip->valid == 0) {
        ...
        ip->nlink = dip->nlink;
        ip->size = dip->size;
        mem_move(ip->addrs, dip->addrs, sizeof(ip->addrs));
...
void iupdate(struct inode *ip) {
    ...
    dip->nlink = ip->nlink;
    dip->size = ip->size;
    mem_move(dip->addrs, ip->addrs, sizeof(ip->addrs));
...
int readi(struct inode *ip, int user_dst, u64 dst, u64 off, u64 n) {
    ...
    for(tot = 0; tot < n; tot += m, off += m, dst += m) {
        ...
        if(user_dst) {
            ...
            if(copy_k2u(curr_proc->page_table, dst, (u64)(bp->data + off % BSIZE), m) < 0) {
                brelse(bp);
                return -1;
            }
        } else {
            mem_move((void*)dst, bp->data + off % BSIZE, m);
        }
...
int writei(struct inode *ip, int user_src, u64 src, u64 off, u64 n) {
    ...
    for(tot = 0; tot < n; tot += m, off += m, src += m) {
        ...
        if(user_src) {
            // 从用户空间复制
            extern struct Process* curr_proc;
            if(copy_u2k(curr_proc->page_table, (u64)(bp->data + off % BSIZE), src, m) < 0) {
                brelse(bp);
                return -1;
            }
        } else {
            mem_move(bp->data + off % BSIZE, (void*)src, m);
        }
